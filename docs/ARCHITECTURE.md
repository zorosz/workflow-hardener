...
[`cmd/find-candidates/main.go`](../cmd/find-candidates/main.go) passes arguments, the `GH_TOKEN` environment value, and output streams to `RunCandidates` in [`candidates.go`](../internal/hardener/candidates.go). It creates a new `candidates.json`, then calls `findCandidates` with a two-minute network deadline and an optional `candidateDebug` writer selected by `--debug`. The exported `FindCandidates` helper uses the same discovery path with debugging disabled. The CLI accepts no search options and does not change the scanner's `Run` or `scan` command.
...
`candidateGET` recognizes rate limits on HTTP 429, or HTTP 403 with a zero remaining allowance or a supplied `Retry-After` header. `candidateRateLimitError` formats the status and validated response-header details into the existing issue message. `candidateRateLimitDetails` shares those validated values with debug logging. Resource names use a fixed allowlist; `candidateRateLimitNumber` accepts one value of at most 12 ASCII digits with a field-specific bound. Counts and delays fit a nonnegative signed 32-bit value, while reset timestamps are bounded to year 9999 and formatted in UTC. Invalid numeric values are omitted, unknown resources are labeled, and absent timing produces `retry time unknown`. No raw response body or request header enters the report. Discovery still stops after the failed request without retrying or making a separate rate-limit check; earlier candidates and exit 2 are preserved.

With `--debug`, `candidateGET` builds a `candidateDebugRecord` for each request and writes it after closing the response body, including elapsed time. Records contain the constructed destination, status or a fixed error category, and selected metadata. `candidateRequestID` allows one short ASCII identifier, and server dates are parsed and normalized to UTC. For non-200 responses only, `candidateDebug.message` reads at most 8 KiB plus one byte, validates UTF-8/JSON, extracts the top-level `message`, redacts the active token, and truncates to 1,024 characters. Invalid message data becomes a fixed unavailable reason without replacing the HTTP failure. Successful bodies stay outside the log.

`candidateDebug.write` JSON-encodes redacted text into single-line records and reserves space for a truncation marker within a 64-KiB total budget. A failed/short write stops logging; `RunCandidates` prints a fixed stdout notice and preserves the discovery report and exit code. A nil debug writer avoids the additional error-body reads. The same logger retains the active token for redaction on anonymous blob requests without sending it to those endpoints.
...
[`find-candidates.yml`](../.github/workflows/find-candidates.yml) exposes **Actions → Find candidates → Run workflow** with an optional **Debug logging** checkbox, defaulting to false. A GitHub-hosted Ubuntu job checks out the project, sets up Go, and builds the discovery executable. The built-in job token is mapped to `GH_TOKEN` only for the discovery step. The boolean input is passed through `DEBUG_LOGGING`; a Bash array selects the literal `--debug` flag. The workflow reuses pinned actions and read-only permissions with checkout credentials unpersisted.
...
When debug is enabled, the discovery step displays `stderr.txt` with Actions command processing disabled via `stop-commands` and a fresh kernel-generated UUID, then restores processing. If a marker cannot be read, log display is skipped and the artifact remains the diagnostic source. The summary points to both locations without embedding response messages into its Markdown. See [GitHub's stop-commands reference](https://docs.github.com/en/actions/reference/workflows-and-actions/workflow-commands#stopping-and-starting-workflow-commands).
...
- [`candidates_test.go`](../internal/hardener/candidates_test.go) uses fake HTTP responses, synthetic tokens, and an injected pacing function. It checks both regex layouts, source attribution, deduplication, metadata validation, sampling and byte limits, partial failures, cancellation, credential handling, and output-file behavior without network calls or real search delays. Rate-limit cases check status/count/time diagnostics, numeric bounds, malformed and duplicate headers, safe handling of unexpected resource names, and retained candidates without further requests. Debug cases cover CLI selection, unchanged reports and requests, error-body and output bounds, redaction before truncation, JSON line boundaries, request IDs, and diagnostic read/write failures.
