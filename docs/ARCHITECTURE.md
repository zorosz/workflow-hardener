...
`candidateGET` recognizes rate limits on HTTP 429, or HTTP 403 with a zero remaining allowance or a supplied `Retry-After` header. `candidateRateLimitError` formats the status and validated response-header details into the existing issue message. Resource names use a fixed allowlist; `candidateRateLimitNumber` accepts one value of at most 12 ASCII digits with a field-specific bound. Counts and delays fit a nonnegative signed 32-bit value, while reset timestamps are bounded to year 9999 and formatted in UTC. Invalid numeric values are omitted, unknown resources are labeled, and absent timing produces `retry time unknown`. No raw response body or request header enters the diagnostic. Discovery still stops after the failed request without retrying or making a separate rate-limit check; earlier candidates and exit 2 are preserved.
